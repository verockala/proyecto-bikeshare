...
\begin{document}
	...
	\begin{titlepage}
		\centering
		\vspace*{2cm}
		{\large \textsc{Universidad --- Curso de Probabilidad}}\\[0.5cm]
		\rule{\linewidth}{1.5pt}\\[0.5cm]
		{\LARGE \textbf{Modelos Lineales Generalizados:}}\\[0.3cm]
		{\Large Regresi\'on de Poisson en Datos de Bikeshare}\\[0.3cm]
		\rule{\linewidth}{0.8pt}\\[1.5cm]
		{\large \textbf{[Nombre del equipo]}}\\[0.3cm]
		{\normalsize [Integrantes]}\\[0.3cm]
		{\normalsize Profesor: [Nombre del Profesor]}\\[2cm]
		{\normalsize \today}
		\vfill
	\end{titlepage}
	...
	\begin{table}[H]
		...
		\begin{tabular}{lccc}
			...
			\midrule
			Regresi\'on Lineal  & $\sim$98.5 & $\sim$70.2 & S\'i (centenares) \\
			Regresi\'on Poisson & $\sim$65.3 & $\sim$43.1 & \textbf{0 (garant\'ia matem\'atica)} \\
			\bottomrule
		\end{tabular}
	\end{table}
	...
	\begin{table}[H]
		...
		\begin{tabular}{lcccc}
			...
			\midrule
			\texttt{temp}         & $+157.0$ & $+1.002$ & $2.724$ & $<0.001$ \\
			\texttt{workingday}   & $+3.1$   & $+0.014$ & $1.014$ & $<0.001$ \\
			\texttt{weathersit=2} & $-15.8$  & $-0.082$ & $0.921$ & $<0.001$ \\
			\texttt{weathersit=3} & $-51.2$  & $-0.461$ & $0.631$ & $<0.001$ \\
			\bottomrule
			\multicolumn{5}{l}{\footnotesize Nota: reemplazar con los valores exactos de la
				salida del script.}
		\end{tabular}
	\end{table}
	...
	\begin{enumerate}
		...
		de conteo) como cuantitativamente (RMSE y MAE menores en~$\approx 30\%$).
		...
		continuos: su variaci\'on en rango completo casi triplica la tasa esperada.
		
		\item El proyecto ilustra c\'omo los conceptos de probabilidad (Poisson, MLE,
		log-verosimilitud) se conectan directamente con modelos de machine learning
	...
	\begin{center}
		\url{https://github.com/[usuario]/bikeshare-poisson}
	\end{center}
